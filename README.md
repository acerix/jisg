...
1. Copy the beginning of the sequence from [The On-line Encyclopedia of Integer Sequences](https://oeis.org/) into a new entry in `OEIS_START_SAMPLES` in [main.test.ts](src/__tests__/main.test.ts.md).  Append an "n" to each integer to make them `BigInt`s (`/(\d+)/\1n/`).
...
1. Push to GitHub and sumbit a [pull request](https://github.com/acerix/jisg/compare).
1. Wait. 🙂
